...
from .model_runtime import predict_from_features, runtime_model_used
...
@app.post("/predict", response_model=AIPredictionResponse)
def predict(request: AIPredictionRequest) -> AIPredictionResponse:
    features = build_features(
        ...
        creatinine_umol_l=request.creatinine_umol_l,
        parser_confidence=request.parserConfidence,
        has_creatinine=request.has_creatinine,
        content_type=request.content_type,
        requires_manual_review=request.requires_manual_review,
    )

    prediction_label, confidence, class_probabilities = predict_from_features(features)
...
def infer_lab_result(request: AiLabAnalysisRequest) -> AiLabAnalysisResponse:
    ...
    try:
        recommendation, model_confidence, _ = predict_from_features(features)
