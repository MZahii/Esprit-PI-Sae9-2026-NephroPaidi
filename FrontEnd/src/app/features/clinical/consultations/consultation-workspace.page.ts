...
export class ConsultationWorkspacePage implements OnInit {
  ...
  // ── Send-to-pharmacy state ────────────────────────────────────────────────
  pharmacyUrgency: 'STAT' | 'URGENT' | 'ROUTINE' = 'ROUTINE';
  pharmacyNotes = '';
  ...
  sendToPharmacy(): void {
    const meds = (this.draft.prescriptions || []).filter(p => p.medication?.trim());
    if (meds.length === 0) {
      this.infoMessage = 'Draft saved. No medications to send to pharmacy.';
      return;
    }

    const user = this.authStorage.getUser();
    const doctorName = [user?.firstName, user?.lastName].filter(Boolean).join(' ') || user?.username || 'Doctor';

    const payload = {
      consultationId: this.consultationId,
      patientId: this.consultation?.patientId ?? undefined,
      patientName: this.consultation?.patientName ?? undefined,
      doctorId: String(this.consultation?.doctorId || user?.keycloakId || ''),
      doctorName,
      urgency: this.pharmacyUrgency,
      medicationsJson: JSON.stringify(meds.map(p => ({
        name: p.medication,
        dosage: p.dosage,
        frequency: p.frequency,
        duration: p.durationDays ? `${p.durationDays} days` : null,
        instructions: p.note ?? '',
        doseMgPerKg: p.doseMgPerKg ?? null
      }))),
      notes: this.pharmacyNotes.trim() || undefined
    };

    this.pharmacySvc.submitPrescription(payload).subscribe({
      next: () => {
        this.infoMessage = 'Draft saved and prescription sent to pharmacy.';
      },
      error: (e: any) => {
        const reason = e?.error?.message || e?.message || 'pharmacy service unreachable';
        this.infoMessage = `Draft saved, but failed to send to pharmacy: ${reason}`;
      }
    });
  }
